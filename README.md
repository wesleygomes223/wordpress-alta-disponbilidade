...
•	Listeners:
o	Porta 80 → Redirecionar para 443.
o	Porta 443 → Encaminhar para o Target Group (Requer certificado do AWS Certificate Manager - ACM).
________________________________________
...
o	Origin domain: Selecionar o ALB.
o	Viewer Protocol Policy: Redirect HTTP to HTTPS.
o	Allowed HTTP Methods: GET, HEAD, OPTIONS, PUT, POST, PATCH, DELETE.
o	Cache Policy: CachingOptimized | Origin request policy: AllViewer.
o	WAF: Habilitar proteções básicas.
o	Custom SSL Certificate: Selecionar o certificado ACM gerado.
3.	Behaviors do CloudFront (Evitar cache no admin do WP):
o	Path /wp-admin/* → Cache Policy: CachingDisabled → Origin Request Policy: AllViewer.
o	Path /wp-login.php → Cache Policy: CachingDisabled → Origin Request Policy: AllViewer.
________________________________________
